...
extern "C"
{
    PGDLLEXPORT PG_FUNCTION_INFO_V1(multicol_topk);
}
...
inline int castAttrToInt(TupleTableSlot *slot, int attno)
{
    bool isNull;
    Datum value = slot_getattr(slot, attno, &isNull);
    int retval = 0;
    ...
    {
    case INT4OID:
        retval = (int)DatumGetInt32(value);
        break;

    case INT8OID:
        retval = (int)DatumGetInt64(value);
        break;

    case FLOAT8OID:
        retval = (int)DatumGetFloat8(value);
        break;

    default:
        elog(ERROR, "castInt unsupported oid = %d", attr->atttypid);
    }
    return retval;
}

bool printtupleslot(TupleTableSlot *slot)
{
    TupleDesc tupdesc = slot->tts_tupleDescriptor;
    int i;
...
///////////////////////////////////////////////////////////////////////////////////////////////////////
// rank_score, index_id
typedef std::pair<float, int> local_pq_item;

struct local_pq_item_compare_gt
{
    bool operator()(const local_pq_item &lhs, const local_pq_item &rhs)
    {
        return lhs.first > rhs.first;
    }
};

struct local_pq_item_compare_lt
{
    bool operator()(const local_pq_item &lhs, const local_pq_item &rhs)
    {
        return lhs.first < rhs.first;
...
TupleTableSlot *execFagins(PlanState *node)
{
    ...
    int num_of_scan = 0;
#ifdef opt
    auto local_pq = new std::priority_queue<local_pq_item, std::vector<local_pq_item>, local_pq_item_compare_gt>();
    auto orderby_id_list = new std::queue<int>();
    const int threshold = 25;
    const int round_num = 25;
    bool first = true;
    float avg_list[2] = {0.0, 0.0};
    int count_list[2] = {0, 0};
#endif
    ...
    while (!estate->finish)
    {
        ...
        if (first)
        {
            // at first, round-robin traverse all indexes
            for (int i = 0; i < threshold; i++)
            {
                for (int orderby_id = 0; orderby_id < estate->children->size(); orderby_id++)
                {
                    orderby_id_list->push(orderby_id);
                }
            }
            first = false;
        }
        else
        {
            ...
            {
                orderby_id_list->push(0);
            }
            else
            {
                orderby_id_list->push(1);
            }
#else
            // then, check top-10 results coming from which index
            for (int i = 0; i < 10; i++)
            {
                int orderby_id = local_pq->top().second;
                int repeat = int(10 / (1 + i)) + 1;
            ...
            // give other index chances
            float weight_list[2];
            weight_list[0] = avg_list[1] / (avg_list[0] + avg_list[1]);
            weight_list[1] = avg_list[0] / (avg_list[0] + avg_list[1]);
            for (int orderby_id = 0; orderby_id < estate->children->size(); orderby_id++)
            {
                int repeat = int(round_num * weight_list[orderby_id]);
                for (int i = 0; i < repeat; i++)
                {
                    orderby_id_list->push(orderby_id);
                }
            }
            // clear local priority queue
            std::priority_queue<local_pq_item, std::vector<local_pq_item>, local_pq_item_compare_gt> null_pq;
            local_pq->swap(null_pq);
#endif
        }

        int queue_size = orderby_id_list->size();
        for (int _ = 0; _ < queue_size; _++)
        {
            int orderby_id = orderby_id_list->front();
            orderby_id_list->pop();
#else
        for (unsigned int orderby_id = 0; orderby_id < estate->children->size(); orderby_id++)
        {
            ...
            {
#endif
                local_pq->push(std::make_pair(rank_score, orderby_id));
                avg_list[orderby_id] = (avg_list[orderby_id] * count_list[orderby_id] + rank_score) / (count_list[orderby_id] + 1);
                count_list[orderby_id]++;
#ifdef one
            }
#endif
#endif
...
#ifdef dev
            int id = castAttrToInt(slot, 1); // get row id
            PRINT("%u %d %f", orderby_id, id, rank_score);
#endif

            consecutive_drops++; // how many round the queue does not change continuously
            if (proc_pq->size() < k || proc_pq->top().first > rank_score)
            {
                if (proc_pq->size() == k)
                    proc_pq->pop(); // pop max score tupe
...
#ifdef opt
    delete (local_pq);
    delete (orderby_id_list);
#endif

    if (!proc_pq->empty())
        PRINT("total scans = %d", num_of_scan);
...
FaginsState *InitFaginsState(Relation relation, uint32_t k, uint32_t term_cond, std::vector<IndexScanState *> scans, std::vector<QueryDesc *> qDescs)
{
...
Datum multicol_topk(PG_FUNCTION_ARGS)
{
    ...
    if (SRF_IS_FIRSTCALL())
    {
        ...
        int argc = 0;
        DEBUG_PRINT("entering multicol_top");
        ...
        if (termCond == 0) // terminate the execution when see how many rounds the queue dose not change
            termCond = 50;
